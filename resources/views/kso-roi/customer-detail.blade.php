...
<div class="container mx-auto px-4 py-6">
            ...
            @forelse($customer->ksoItems as $item)
                    <div class="bg-white border border-gray-200 rounded-lg p-6 mb-4 shadow-sm">
                        ...
                        <div class="mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                            ...
                            <div id="qr-code-{{ $item->id }}" class="hidden mt-3 pt-3 border-t border-purple-200">
                                <div class="flex flex-col items-center gap-3">
                                    <!-- QR Code Image -->
                                    <div class="p-2 bg-white rounded-lg border border-purple-300">
                                        <img 
                                            src="{{ \App\Helpers\QrCodeHelper::generateKsoItemQrCode($item->id, 200) }}" 
                                    ...
                                    <div class="w-full text-center">
                                        ...
                                        <div class="flex gap-2 justify-center flex-wrap">
                                            <a 
                                                ...
                                                target="_blank"
                                                class="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition text-xs font-medium"
                                            >
                                            ...
                                            <button 
                                                ...
                                                class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-xs font-medium"
                                            >
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Edit Customer Modal -->
    <div id="editCustomerModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-lg bg-white">
            ...
            <form action="{{ route('kso-roi.customer.update', $customer->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="border-b border-gray-200 mb-6">
                    <nav class="-mb-px flex space-x-8">
                        <button type="button" onclick="switchTab('customer-info')" id="customer-info-tab" 
                                class="py-2 px-1 border-b-2 border-blue-500 font-medium text-sm text-blue-600">
                            Informasi Customer
                        </button>
                        <button type="button" onclick="switchTab('equipment')" id="equipment-tab" 
                            ...
                            Alat & Peralatan ({{ $customer->ksoItems->count() }})
                        </button>
                    </nav>
                </div>

                <!-- Customer Info Tab -->
                <div id="customer-info-content" class="tab-content">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" rows="3" 
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">{{ $customer->address ?? '' }}</textarea>
                        </div>
                ...
                <div id="equipment-content" class="tab-content hidden">
                    <div class="mb-4 flex justify-between items-center">
                        <h4 class="text-lg font-medium text-gray-900">Daftar Alat & Peralatan</h4>
                        <a href="{{ route('kso-roi.create-kso-item') }}?customer_id={{ $customer->id }}" 
                           class="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors inline-flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                            </svg>
                            Tambah Alat Baru
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-2">
                                                <h5 class="font-medium text-gray-900">{{ $item->nama_alat }}</h5>
                                                <span class="px-2 py-1 text-xs font-medium rounded {{ $item->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                    {{ ucfirst($item->status) }}
                                                </span>
                                                @if($item->kategori)
                                            ...
                                            <div class="text-sm text-gray-600 space-y-1">
                                                @if($item->brand && $item->model)
                                                    <div><span class="font-medium">Brand/Model:</span> {{ $item->brand }} - {{ $item->model }}</div>
                                                @endif
                                                @if($item->serial_number)
                                                    <div><span class="font-medium">Serial Number:</span> {{ $item->serial_number }}</div>
                                                @endif
                                                @if($item->lokasi_penempatan)
                                                    <div><span class="font-medium">Lokasi:</span> {{ $item->lokasi_penempatan }}</div>
                                                @endif
                                                <div><span class="font-medium">Investasi:</span> Rp {{ number_format($item->total_investasi, 0, ',', '.') }}</div>
                                                @if($item->supportItems->count() > 0)
                                                    <div><span class="font-medium">Alat Pendukung:</span> {{ $item->supportItems->count() }} unit</div>
                                                @endif
                                            </div>
                                        </div>
                                        
                                        <div class="flex gap-2 ml-4">
                                            <a href="{{ route('kso-roi.edit-kso-item', $item->id) }}" 
                                            ...
                                            <form method="POST" action="{{ route('kso-roi.destroy-kso-item', $item->id) }}" 
                    ...
                    @else
                        <div class="text-center py-8 bg-gray-50 rounded-lg">
                            ...
                            <p class="text-gray-500">Belum ada alat untuk customer ini</p>
                            <a href="{{ route('kso-roi.create-kso-item') }}?customer_id={{ $customer->id }}" 
