...
<div class="bg-white rounded-lg shadow p-6 mb-6 overflow-x-auto">
    <div class="flex items-center justify-between mb-6 min-w-max">
        <div>
        ...
        <div class="flex items-center space-x-3 mb-4">
            <label for="productFilter" class="text-sm font-medium text-gray-700">Filter Produk:</label>
            <select id="productFilter" multiple class="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" style="width: 300px;">
                <option value="">Semua Produk</option>
                @foreach($products as $product)
                    <option value="{{ $product->id }}">{{ $product->name }}</option>
                @endforeach
            </select>
            <button id="clearFilter" class="px-3 py-1 bg-gray-500 text-white rounded-md text-sm hover:bg-gray-600">
                Reset
            </button>
        </div>
    ...
    <div class="relative" style="height: 1000px; min-width: 800px;">
...
<div class="bg-white rounded-lg shadow p-6 mb-6 overflow-x-auto">
    ...
    <div class="relative" style="height: 600px; min-width: 800px;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let originalChartData = JSON.parse(JSON.stringify(@json($chartData)));

    // Initialize Select2
    $(document).ready(function() {
        $('#productFilter').select2({
            placeholder: "Pilih produk untuk ditampilkan",
            allowClear: true,
            width: '300px'
        });
    ...
    function filterChart() {
        const selectedValues = $('#productFilter').val() || [];
        
        console.log('Selected values:', selectedValues);
        console.log('Original datasets:', originalChartData.datasets.length);
        
        if (selectedValues.length === 0) {
            // Show all products
            chart.data.datasets = originalChartData.datasets;
            console.log('Showing all products:', chart.data.datasets.length);
        } else {
            // Show only selected products
            const filteredDatasets = originalChartData.datasets.filter(dataset => {
                // Find product by ID and check if name matches
                return selectedValues.some(productId => {
                    const option = Array.from(productFilter.options).find(opt => opt.value === productId);
                    const matches = option && option.text === dataset.label;
                    if (matches) {
                        console.log('Match found:', dataset.label);
                    }
                    return matches;
                });
            });
            chart.data.datasets = filteredDatasets;
            console.log('Filtered datasets:', chart.data.datasets.length);
        }
