...
@section('title', 'QC / Kalibrasi - ' . $ksoItem->nama_alat)
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">
                    {{ $type === 'qc' ? '🔍 Quality Control' : '🔧 Kalibrasi' }}
                </h1>
                <p class="text-gray-600 mt-2">{{ $ksoItem->nama_alat }} - {{ $ksoItem->customer->name }}</p>
            </div>
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                <p class="text-sm text-blue-700">
                    <strong>{{ $type === 'qc' ? 'QC' : 'Kalibrasi' }} terakhir:</strong> 
                    {{ $lastRecord->date->format('d M Y') }} - 
                    Status: <span class="font-semibold">{{ $lastRecord->status_text }}</span>
                    @if($lastRecord->next_due_date)
                        - Selanjutnya jatuh tempo: {{ $lastRecord->next_due_date->format('d M Y') }}
                    @endif
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">
                Form {{ $type === 'qc' ? 'Quality Control' : 'Kalibrasi' }}
            </h2>
        </div>
        
        <form action="{{ route('kso-roi.qc.store', [$ksoItem, $type]) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="date" class="block text-sm font-medium text-gray-700 mb-2">
                            Tanggal {{ $type === 'qc' ? 'QC' : 'Kalibrasi' }} <span class="text-red-500">*</span>
                        </label>
                        <input type="date" 
                               id="date" 
                               name="date" 
                            ...
                            <div>
                                <label for="next_due_days" class="block text-sm font-medium text-gray-700 mb-1">
                                    ...
                                    <small class="text-gray-500">
                                        ({{ $type === 'qc' ? 'Default: 14 hari' : 'Default: 30 hari' }})
                                    </small>
                                </label>
                                <input type="number" 
                                       id="next_due_days" 
                                       name="next_due_days" 
                                       value="{{ old('next_due_days', $type === 'qc' ? 14 : 30) }}"
                                       min="1"
                                       max="365"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <p class="mt-1 text-xs text-gray-500">
                                    Masukkan jumlah hari dari tanggal {{ $type === 'qc' ? 'QC' : 'kalibrasi' }} hingga jatuh tempo berikutnya
                                </p>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="notes" 
                                  ...
                                  rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Deskripsi hasil {{ $type === 'qc' ? 'QC' : 'kalibrasi' }}, temuan, rekomendasi...">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">{{ $ksoItem->nama_alat }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Brand/Model:</span>
                                <span class="font-medium">{{ $ksoItem->brand }} {{ $ksoItem->model }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Serial Number:</span>
                                <span class="font-medium">{{ $ksoItem->serial_number }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Lokasi:</span>
                                <span class="font-medium">{{ $ksoItem->lokasi_penempatan }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg">
                        ...
                        <div class="space-y-2">
                            <button type="button" 
                                    onclick="window.open('https://wa.me/?text={{ urlencode("Halo, saya ingin melakukan " . ($type === 'qc' ? 'QC' : 'kalibrasi') . " untuk alat " . $ksoItem->nama_alat . " di " . $ksoItem->customer->name) }}')"
                                    class="w-full bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded text-sm">
                                <i class="fab fa-whatsapp mr-2"></i> Notifikasi Customer
                            </button>
                            <button type="button" 
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 mt-6">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i> Simpan {{ $type === 'qc' ? 'QC' : 'Kalibrasi' }}
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateDueDate() {
        ...
        if (date && days > 0) {
            ...
            dueDateDisplay.textContent = `Due date: ${formattedDate}`;
    ...
    function calculateDays() {
        ...
        if (date && dueDate) {
            ...
            if (diffDays >= 0) {
                ...
                daysDisplay.textContent = `Perhatian: Tanggal due date harus setelah tanggal ${type === 'qc' ? 'QC' : 'kalibrasi'}`;
