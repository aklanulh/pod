...
class ImportSuppliersImport implements ToModel, WithHeadingRow, WithValidation, SkipsOnError, SkipsOnFailure, SkipsEmptyRows
{
    private $rowCount = 0;
    ...
    public function model(array $row)
    {
        try {
            ...
            Log::error('Error processing supplier row: ' . json_encode($row));
    ...
    {
        Log::error('ImportSuppliersImport Error: ' . $e->getMessage());
    ...
    public function onFailure(\Maatwebsite\Excel\Validators\Failure ...$failures)
    {
        foreach ($failures as $failure) {
            $this->skippedRows++;
            Log::error('ImportSuppliersImport Validation Failure:');
